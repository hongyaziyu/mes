        ...
        <form id="form" class="form-inline" action="${pageContext.request.contextPath}/plan/addProductTest.action" method="post">
            <table class="table table-hover">
              <tr>
                   ...
                   <td><input  type="text" class="form-control" name="planNo"  value="${listProductTest[0].planNo}"></td>
              ...
              <tr>
                   ...
                   <td><input  type="text" class="form-control" name="batchNo"  value="${listProductTest[0].batchNo}"></td>
              </tr>
          </table>
        <h4>产品测量记录</h4>
        <div id="div">
        	<jsp:include page="productTestDiv1.jsp"></jsp:include>
        </div>
                <br />
	...
	//物料号：根据图号得到对应的物料号，并下拉框显示
	 $("#wl").mousemove(function(){
		...
		$("#tijiao").click(function(){
			var j=1;
			$(".used").each(function(){
				var produce_date = $(this).children().first().next().children().first().children().first().children().first().children().first().children().first();	
				produce_date.attr("name","produce_date" + j);				
				var gongxu = produce_date.parent().parent().next().children().first();				
				...
				var clz3 =clz2.parent().parent().next().children().first().children().first();
				clz3.attr("name","clz3" + j);
				var clz4 = clz3.parent().next().children().first();
